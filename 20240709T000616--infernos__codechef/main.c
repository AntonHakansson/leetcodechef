/*
#+title: Inferno
#+ref: https://www.codechef.com/practice/course/1-star-difficulty-problems/DIFF1200/problems/INFERNO
...
static void run(Arena arena, Stream *reader, Stream *writer) {
  ...
  for (Size t = 0; t < T; t++) {
    ...
    Size X = read_i64(reader);

    /*
      r_single: \[ \sum_{\forall i} \lceil \frac{h_i - 1}{X} \rceil \]
      r_multi:  \[ \max_{\forall i} h_i   \]
     */
    U64 r_single = 0;
    U64 r_multi = 0; {
      U64 max_h = 0;
      for (Size i = 0; i < N; i++) {
        U64 h_i = read_i64(reader);
        max_h = h_i > max_h ? h_i : max_h;
        r_single += ((h_i - 1) / X) + 1;
      }
      r_multi = max_h;
    }
    write_i64(writer, r_single < r_multi ? r_single : r_multi, '\n');
  }
}
