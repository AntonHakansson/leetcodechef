/*
#+title: Large Square
#+ref: https://www.codechef.com/practice/course/1-star-difficulty-problems/DIFF1200/problems/XLSQUARE
...
static void run(Arena arena, Stream *reader, Stream *writer) {
  ...
  for (Size t = 0; t < T; t++) {
    ...
    Size A = read_i64(reader);
    Size root = __builtin_sqrt(N);
    Size result = (root < 1 ? 1 : root) * A;
    write_i64(writer, result, '\n');
  }
}
